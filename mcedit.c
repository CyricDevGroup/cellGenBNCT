...
/*** filetypes ***/              //, ".c", ".h", ".cpp"
char *C_HL_extensions[] = { ".i",".inp", NULL }; //these are the filetypes for now
...
void disRM() {
  if(tcsetattr(STDIN_FILENO, TCSAFLUSH, &cVar.orig_termios) == -1) die("tcsetattr");

}

void enRM() {

  if(tcgetattr(STDIN_FILENO, &cVar.orig_termios) == -1) die("tcgetattr");
  atexit(disRM);
  ...
  raw.c_cflag |= (CS8);
                   ...
                   //playing with bits
  raw.c_lflag &= ~(ECHO | ICANON | IEXTEN | ISIG);   //turn off canonical mode GNU ISIG disable ctrl-c or ctrl-z and etc...
  ...
  if(tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw) == -1) die("tcsetattr");  //set attributes
}
  ...
  if (c == '\x1b') {
    ...
    if (seq[0] == '[') {
      if (seq[1] >= '0' && seq[1] <= '9') {
        ...
        if (seq[2] == '~') {
          switch (seq[1]) {
            case '1': return HOME_KEY;  //<esc>[1~  these are VT100 codes for Home  key
            case '3': return DEL_KEY;   //<esc>[3~  these are VT100 codes for Del   key
            case '4': return END_KEY;   //<esc>[4~  these are VT100 codes for End   key
            case '5': return PAGE_UP;   //<esc>[5~  these are VT100 codes for PageU key
            case '6': return PAGE_DOWN; //<esc>[6~  these are VT100 codes for PageD key
            case '7': return HOME_KEY;  //<esc>[7~  these are VT100 codes for Home  key
            case '8': return END_KEY;   //<esc>[8~  these are VT100 codes for End   key
          }
        }
      } else {
        switch (seq[1]) {
          case 'A': return ARROW_UP;     //<esc>[A  The arrow key codes
...
int winsizeFunc(int *rows, int *cols) {
  ...
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
    if (write(STDOUT_FILENO, "\x1b[999C\x1b[999B", 12) != 12) return -1; //we send two escape sequence here, C moves the mouse right forward and B moves the mouse down, the value 999 is rather large so to ensurewe reached edge of the screen
...
void mceditrowrefup(erow *row) {
  ...
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % mceditFORT_TAB_STOP != 0) row->render[idx++] = ' ';
    } else {
...
void mceditDelRow(int at) {
  ...
  memmove(&cVar.row[at], &cVar.row[at + 1], sizeof(erow) * (cVar.Nrow - at - 1));
  cVar.Nrow--;
  cVar.modfycnt++;
...
void mceditRowInsertChar(erow *row, int at, int c) {
  ...
  memmove(&row->chars[at + 1], &row->chars[at], row->size - at + 1); //make room for new char
  row->size++;
  row->chars[at] = c;
  mceditrowrefup(row);
    cVar.modfycnt++;
...
void mceditDelChar() {
  if (cVar.cy == cVar.Nrow) return;
    if (cVar.cx == 0 && cVar.cy == 0) return;
  erow *row = &cVar.row[cVar.cy];
  if (cVar.cx > 0) {
...
void mceditSave() {
  ...
  int fd = open(cVar.filename, O_RDWR | O_CREAT, 0644); //O_CREAT open existing file, O_RDWR read and write, 0644 standard permission
  if (fd != -1) {
...
void mceditscreenref() {   //note 4 as there 4 args \x1b, [, 2, J
  write(STDOUT_FILENO, "\x1b[2J", 4); //\x1b (decimal 27) escape character, J to clear screen
  write(STDOUT_FILENO, "\x1b[H", 3); //cursor position on screen

  mceditDrawRows();

  write(STDOUT_FILENO, "\x1b[H", 3);
}                                    //[2J clear the entire screen [1J up to cursor point
*/

void mceditstatbardraw(struct abuf *ab) {  //the status bar, <esc>[7m escape seq for inverted colors, <esc>[m normal
  ...
  int rlen = snprintf(rstatus, sizeof(rstatus), "%s | %d/%d",  //show filetype in statusbar
    cVar.syntax ? cVar.syntax->filetype : "no ft", cVar.cy + 1, cVar.Nrow);

//  int rlen = snprintf(rstatus, sizeof(rstatus), "%d/%d",
//     cVar.cy + 1, cVar.Nrow);
  if (len > cVar.screencols) len = cVar.screencols;
...
void mceditstatbarsetup(struct abuf *ab) {
  ...
  if (msglen > cVar.screencols) msglen = cVar.screencols;
  if (msglen && time(NULL) - cVar.statusmsg_time < 5) //after 5 seconds, when keys are pressed, screen refreshes by keypress
...
void mceditDrawRows(struct abuf *ab) {
  ...
  for (y = 0; y < cVar.screenrows; y++) {
    ...
    if (filerow >= cVar.Nrow) {
      ...
      //  char welcome[80];
        char mssg[100];
        int mssglen = snprintf(mssg, sizeof(mssg),
          "mcedit PHITS editor Developed by Mehrdad S.Beni, Hiroshi Watabe & Peter K.N. Yu -- version %s", mceditFORT_VER);
        if (mssglen > cVar.screencols) mssglen = cVar.screencols;
      ...
      for (j = 0; j < len; j++) {
        if (hl[j] == HL_NORMAL) {
          if (current_color != -1) {
            abAppend(ab, "\x1b[39m", 5); //<esc>[39m to make sure we’re using the default text color
            current_color = -1;
          }
          abAppend(ab, &c[j], 1);
        } else {
...
void mceditscreenref() {
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (cVar.cy - cVar.rowoff) + 1, cVar.cx + 1);
...
void mceditkeypress() {
  ...
  mceditstatbarmsgset("unsaved changes to your phits script. "
    "Press ctrl-q %d more time to quit.", QTcount);
  QTcount--;
  return;
}
      write(STDOUT_FILENO, "\x1b[2J", 4);
    ...
    case CTRL_KEY('h'):
    case DEL_KEY:
    if (c == DEL_KEY) mceditcursmov(ARROW_RIGHT);
mceditDelChar();
      break;
    case PAGE_UP:
    case PAGE_DOWN:
      {
        if (c == PAGE_UP) {
          ...
          if (cVar.cy > cVar.Nrow) cVar.cy = cVar.Nrow;
        }
...
void initmcedit() {
  ...
  //cVar.screenrows +=1;
  if (winsizeFunc(&cVar.screenrows, &cVar.screencols) == -1) die("winsizeFunc");
